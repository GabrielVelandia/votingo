	...
	class Login
	{
		...
		public function passwordRecover($data)
		{
			try{
                ...
                INNER JOIN statuses s ON s.id = u.status_idfk
				INNER JOIN roles r ON r.id = ur.role_idfk
				WHERE u.document= '{$data['document']}' AND u.email = '{$data['email']}'";
				...
				if (isset($query[0]->id)) {
					...
					$_SESSION['user'] = $query[0];
					return 'Hemos enviado un correo a tu buzón de entrada, revisalo ahora mismo :D.';
					
				} else {

					return 'No se encontraron coincidencias en nuestra base de datos, verifique los datos e intente de nuevo.';
