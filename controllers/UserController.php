<?php
	...
	class UserController
	{
		...
		public function __construct()
		{
			...
			$this->sList = new Student_listing;
		...
		{	
			...
			try {
				...
				$candidacyS = [
					'candidateType' => $_POST['candidatetype_idfk'],
					'studentList' => $_POST['studentlist_idfk'],
				    // 'photo' => addslashes(file_get_contents($_FILES['photo']['tmp_name'])),
					$enviar
				];
				$tipoArchivo = $_FILES['photo']['type'];
				$permitido = array("image/png","image/jpg","image/jpeg", "image/svg");
				if( in_array($tipoArchivo,$permitido) == false && $tipoArchivo != '' ){
				?>
				<script>
					alert("Archivo no permitido");
					window.location="?controller=user&method=indexStudent";
				</script>
				<?php
				die();
				}
				...
				$this->model->newStudentCandidacy($candidacyS,$foto);
				$role = 4;
				$this->model->newRole($role,$lastIdUser);
		...
		public function updateStatus()
	{
		...
		public function updateStatusStudent()
	{
		$users = $this->model->getUserById($_REQUEST['id']);
		$data = [];
		if ($users[0]->status_idfk == 1) {
    ...
    public function resetpasswordChange()
   	{
        $currentPassword = $_POST['currentPassword'];
        $newPassword = $_POST['newPassword'];
        /*
        var_dump($currentPassword,$newPassword);
        ...
        */
        if ($currentPassword == $newPassword && $currentPassword != '' && $newPassword != '') {
            $updateChangePassword = $this->model->updateChangePassword($newPassword,$currentPassword);
            header('Location: ?controller=login&method=logout');
        }else{
        	$updateChangePassword = $this->model->updateChangePassword($newPassword,$currentPassword);
        $error = [
			'errorMessage' => $updateChangePassword
		];

        }
		require "views/passwordChange.php";
    }
}
